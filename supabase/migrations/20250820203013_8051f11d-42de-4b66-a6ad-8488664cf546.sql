...
-- Create new label_templates table with different structure than existing one
...
-- Enable RLS
ALTER TABLE label_templates_new ENABLE ROW LEVEL SECURITY;
ALTER TABLE print_jobs ENABLE ROW LEVEL SECURITY;

-- RLS policies for label_templates_new
CREATE POLICY "Anyone can view label_templates_new" ON label_templates_new
  FOR SELECT USING (true);

CREATE POLICY "Staff/Admin can insert label_templates_new" ON label_templates_new
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'staff'::app_role) OR has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Staff/Admin can update label_templates_new" ON label_templates_new
  FOR UPDATE USING (has_role(auth.uid(), 'staff'::app_role) OR has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Staff/Admin can delete label_templates_new" ON label_templates_new
...
CREATE POLICY "Anyone can insert print_jobs_new" ON print_jobs
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Staff/Admin can view print_jobs_new" ON print_jobs
  FOR SELECT USING (has_role(auth.uid(), 'staff'::app_role) OR has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Staff/Admin can update print_jobs_new" ON print_jobs
  FOR UPDATE USING (has_role(auth.uid(), 'staff'::app_role) OR has_role(auth.uid(), 'admin'::app_role));

-- Insert default 2x1 price label template
INSERT INTO label_templates_new (id, body, required_fields, optional_fields) VALUES (
  'price-2x1-v1',
